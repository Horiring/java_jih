...
public class BaseballGameEx {
	...
	public static void main(String[] args) {
		...
		Array.printArray(com);//나중에 주석으로 숨김
		...
		do {
			...
			if(Array.arrayCheck(user)) {
				System.out.println("중복된수를 입력했습니다.");
				continue;
			}
	...
	/**	스트라이크 개수를 판별하는 메서드
	 * 	=> 두 배열에서 같은 번지에 있는 값들이 몆개 같은지 알려주는 메서드
	 *  매개변수 :	두 배열 =>int arr1[], int arr2[]
	 *  리턴타입 :	같은 번지에 있는 값들이 몆개 같은지 => 몆개 => 정수 => int
	 *  메서드명 : strike
	 */
	...
	/**	볼 개수를 판별하는 메서드
	 * 	=> 두 배열에서 다른 번지에 있는 값들이 몆개 같은지 알려주는 메서드
	 *  매개변수 :	두 배열 =>int arr1[], int arr2[]
	 *  리턴타입 :	같은 번지에 있는 값들이 몆개 같은지 => 몆개 => 정수 => int
	 *  메서드명 : ball
	 */
	...
	public static int ball(int arr1[],int arr2[]) {
		//배열 생성자 인원 배열이 1라도 있으면 비교할수 없어서 0을 리턴
		if(arr1 == null || arr2 == null) {
			return 0;
		}
		int count= 0;
